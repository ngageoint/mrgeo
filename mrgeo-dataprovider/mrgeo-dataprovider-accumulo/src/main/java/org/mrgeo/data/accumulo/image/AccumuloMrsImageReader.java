 ...
 * @param numQueryThreads the number of threads for multithreaded queries
 * @param instance        the accumulo instance in ZooKeeper
 * @param zooServers      the ZooKeepers to use
 * @param user            the user to use to connect to accumulo
 * @param pass            the password to use to connect to accumulo
 */
...
public MrGeoRaster get(final TileIdWritable key)
{
  ...
  try
  {
    if (it.hasNext())
    {
      ...
      log.debug(
          "tid = " + ent.getKey().getColumnQualifier().toString() + " image byte size = " + ent.getValue().getSize());
 ...
 *
 * @param startKey the start of the list of items to pull
 * @param endKey   the end (inclusive) of items to pull
    ...
    final TileIdWritable endKey)
{
  ...
  if (connector == null)
  {
    ...
    catch (DataProviderException ignored)
    {
