...
parser.add_argument("cost",
                    help="Specify whether you want to use on demand pricing or spot bid pricing. Defaults to on demand.",
                    choices=['spot', 'ondemand'], action="store", default="ondemand")
parser.add_argument("-n", "--nodetype", help="AWS instance type, m3.xlarge is default.", action="store",
                    default="m3.xlarge")
parser.add_argument("-z", "--zone", help="AWS availability zone, us-east-1a is default.", action="store",
                    default="us-east-1a")
parser.add_argument("type", nargs='?',
                    help="Run the full suite of benchmarks (full) or run individual ones (partial). Will take a very long time for full. If partial please specify which benchmarks to run. Defaults to partial.",
                    choices=['full', 'partial'], action="store", default="partial")
parser.add_argument("-x", "--noterminate",
                    help="Do not automatically terminate the cluster when all steps have completed. Default behavior is to auto terminate. Use this flag to override.",
                    action="store_true")
...
parser.add_argument("-lc", "--landcover",
                    help="Benchmark nested conditional statment converting land cover data to impedance values.",
                    action="store_true")
parser.add_argument("-fs", "--frictionsurface", help="Benchmark friction surface map algebra.", action="store_true")
parser.add_argument("-l", "--launchonly",
                    help="Spin up a MrGeo cluster with WMS server on port 8080 and leave running.", action="store_true")
args = parser.parse_args()

if args.cost == 'spot':
    cprice = subprocess.check_output(
        'aws ec2 describe-spot-price-history --instance-types ' + args.nodetype + ' --availability-zone ' + args.zone + ' --start-time ' + nowf + ' --end-time ' + nowf + ' --product-descriptions Linux/UNIX | awk \'{print $5}\'',
...
if args.type == 'full' or args.launchonly:
    ...
    if all([not args.ingestimage, not args.rasterizevector, not args.buildpyramid, not args.slope, not args.landcover,
            not args.frictionsurface]):
