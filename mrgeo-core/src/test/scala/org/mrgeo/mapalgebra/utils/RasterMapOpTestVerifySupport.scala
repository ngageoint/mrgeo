...
trait RasterMapOpTestVerifySupport extends RasterMapOpTestSupport {
  ...
  def verifyRasters(rdd:RDD[_], verifiers:RasterVerifiers, failOnMissingKey:Boolean = true):Unit = {
    ...
    if (failOnMissingKey && idsToVerify.size != 0) {
      Assertions.fail("Keys missing from RDD: " + idsToVerify.mkString)
    }
  }
  ...
  def verifyRastersAreTheSame(expected:MrGeoRaster)(actual:MrGeoRaster):Unit = {
    ...
    Assertions.assertResult(width, "Raster width") {
      actual.width()
    }
    Assertions.assertResult(height, "Raster height") {
      actual.height()
    }
    Assertions.assertResult(bands, "Raster number of bands") {
      actual.bands()
    }

    forEachSampleInRaster(actual, (b, x, y, sample) => {
      Assertions.assertResult(expected.getPixelDouble(x, y, b), s"Sample at x: $x y: $y band: $b") {
        sample
      }
    })
  }
  ...
  def verifyRasterNoData(tileId:Long, zoomLevel:Int, tileSize:Int, raster:MrGeoRaster, nodatas:Array[Double],
    ...
    println(s"For Tile $tileId LeftBottom bounds pixel: ${pixelLeftBottom.px}, ${
      pixelLeftBottom.py
    }. RightTop bounds pixel: ${pixelRightTop.px}, ${pixelRightTop.py}")

    val verifyExpectedData = expectedData match {
      case Some(dataArray) => (b:Int, x:Int, y:Int, sample:Double) => {
        Unit
        if (b < dataArray.size) {
          Assertions.assertResult(dataArray(b), s"Sample at x: $x y: $y band: $b") {
            sample
          }
        }
      }
    ...
    def verifySample(b:Int, x:Int, y:Int, sample:Double):Unit = {
      (x, y) match {
        case (x, y) if x < pixelLeftBottom.px || x > pixelRightTop.px ||
          ...
          Assertions.assertResult(nodatas(b), s"Sample at x: $x y: $y band: $b") {
            sample
          }
