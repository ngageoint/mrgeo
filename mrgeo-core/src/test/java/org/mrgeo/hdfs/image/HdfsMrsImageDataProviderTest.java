...
public void testGetResourceLocal() throws Exception
{
  ...
  Assert
      .assertEquals("Resolved Resource name is wrong!", "file://" + name,
          local.getResourcePath(true).toUri().toString());
}
...
public void testGetTiledOutputFormatProvider() throws Exception
{
  ImageOutputFormatContext context =
      new ImageOutputFormatContext("foo", null, 10, MrGeoConstants.MRGEO_MRS_TILESIZE_DEFAULT_INT, "");
