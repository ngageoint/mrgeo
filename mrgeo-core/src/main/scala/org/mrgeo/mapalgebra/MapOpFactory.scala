...
@SuppressFBWarnings(value = Array("NP_LOAD_OF_KNOWN_NULL_VALUE", "RCN_REDUNDANT_NULLCHECK_OF_NONNULL_VALUE"),
  justification = "not really errors, scan flags them in one case, but not another")
@SuppressFBWarnings(value = Array("PATH_TRAVERSAL_IN"), justification = "decendants() - Filenames come from classpath")
@SuppressFBWarnings(value = Array("BC_UNCONFIRMED_CAST_OF_RETURN_VALUE"),
  justification = "getSignatures(), decendants() - Scala generated code")
  ...
  private def decendants(clazz:Class[_]) = {
    ...
    val urls = (ClasspathHelper.forClassLoader() ++ ClasspathHelper.forJavaClassPath()).filter(url => {
      ...
      (if (MrGeoProperties.isDevelopmentMode) {
        file.getName.contains("mrgeo") || file.getName.equals("__app__.jar")
      }
      else {
        file.isFile
      })
