...
public static void copyFile(String srcFileName, String dstFileName) throws Exception
{
  ...
  {
    try (OutputStream out = new FileOutputStream(dstFileName))
    {
...
//      pos = name.indexOf(".");
      ...
      if (prefix != null)
      {
        System.out.println(prefix + aChild.getCanonicalPath() + " -> " + dir2 + "\\"
            + newBasename + "." + ext);
      }
      copyFile(aChild.getCanonicalPath(), dir2 + "\\" + newBasename + "." + ext);
      if (move)
      {
        if (!aChild.delete())
        {
