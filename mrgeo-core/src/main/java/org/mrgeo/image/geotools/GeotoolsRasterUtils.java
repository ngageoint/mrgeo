...
public class GeotoolsRasterUtils
{
  ...
  {
    final Integer borderSizeInt = new Integer(borderSize);
      ...
      throws Exception
  {
    ...
    {
      log.info("   load data for : " + input.toString());

      ImageIngestDataProvider provider = DataProviderFactory.getImageIngestDataProvider(input,
          AccessMode.READ);
...
//      geotoolsImage = f.create("foo", wr, envelope);
  ...
  public static AbstractGridCoverage2DReader openImage(final String image) throws IOException
  {
    ...
    log.debug("Loading Image file: " + image.toString());
    try
    {

      ImageIngestDataProvider provider = DataProviderFactory.getImageIngestDataProvider(image,
          AccessMode.READ);
      final InputStream stream = provider.openImage();
