...
/**
 * Created by ericwood on 8/16/16.
 */
public class GeoserverPublisherConfigurator implements MrGeoPublisherConfigurator {
    ...
    private static final String DEFAULT_COVERAGE_STORE_DESCRIPTION = "MrGeo data source, automatically (periodically) updated with new layers";
    ...
    public void configure(MrGeoPublisher publisher, Properties profileProperties) {
        ...
        if (geoserverUsername == null || geoserverPassword == null) {
            throw new IllegalArgumentException("Geoserver username and password must be specified to configure Geoserver Publisher");
        }
        ...
        geoserverPublisher.setCoverageStoreName(profileProperties.getProperty(COVERAGE_STORE_NAME_PROPERTY, DEFAULT_COVERAGE_STORE_NAME));
