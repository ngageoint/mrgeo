...
public class RasterUtils
{
  ...
  {
    ColorModel colorModel = PlanarImage.getDefaultColorModel(dataType, numBands);
    if (colorModel == null)
    {
  ...
  public static ColorModel createColorModel(final Raster raster)
  {
    ...
    ColorModel cm = PlanarImage.getDefaultColorModel(type, raster.getNumBands());

    if (cm == null)
    {
      if (type == DataBuffer.TYPE_DOUBLE || type == DataBuffer.TYPE_FLOAT)
      {
        cm = new FloatDoubleColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY), false,
            false, Transparency.OPAQUE, type);
      }
      else
      {
  ...
  {
    SampleModel sm = new BandedSampleModel(dataType, basedOnSource.getWidth(), basedOnSource.getHeight(), numBands);
    return new ImageLayout(basedOnSource.getMinX(),
        basedOnSource.getMinY(),
        basedOnSource.getWidth(),
        basedOnSource.getHeight(),
        basedOnSource.getTileGridXOffset(),
        basedOnSource.getTileGridYOffset(),
        basedOnSource.getTileWidth(),
        basedOnSource.getTileHeight(),
        sm,
