...
{
  String exp =
      "RasterizePoints([" + hdfsShapefile + "], \"SUM\", 0.0001716614, \"" + column + "\", 68.85, 34.25, 69.35, 34.75)";
  if (GEN_BASELINE_DATA_ONLY)
  {
    testUtils.generateBaselineTif(this.conf, testname.getMethodName(), exp, -9999);
  }
...
{
  String exp =
      "RasterizePoints([" + hdfsShapefile + "], \"SUM\", 0.0001716614, \"" + column + "\", [" + hdfsCropRaster + "])";
  if (GEN_BASELINE_DATA_ONLY)
  {
    testUtils.generateBaselineTif(this.conf, testname.getMethodName(), exp, -9999);
  }
...
{
  String exp =
      "RasterizePoints([" + hdfsShapefile + "], \"SUM\", 0.0001716614, \"" + column + "\", 68.85, 34.25, 69.35)";
  MapAlgebra.validateWithExceptions(exp, ProviderProperties.fromDelimitedString(""));
}
...
{
  String exp = "RasterizePoints([" + hdfsShapefile + "], \"BAD\", 0.0001716614, \"" + column +
