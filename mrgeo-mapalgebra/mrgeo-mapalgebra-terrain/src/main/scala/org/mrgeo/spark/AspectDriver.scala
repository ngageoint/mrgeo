...
object AspectDriver extends MrGeoDriver with Externalizable {

  def aspect(input:String, units:String, output:String, conf:Configuration) = {
    val args =  mutable.Map[String, String]()

    val name = "Aspect (" + input + ")"

    args += SlopeAspectDriver.Input -> input
    args += SlopeAspectDriver.Units -> units
    args += SlopeAspectDriver.Output -> output
    args += SlopeAspectDriver.Type -> SlopeAspectDriver.Aspect
