...
public static List<Long> getSequential(String keyFile, String[] splits, int num) {
  ...
  try {
    ...
    for(int i=0; i < splits.length; i++) {
      keys = findSequentialRange(allKeys, num, Long.parseLong(splits[i]), (i < splits.length-1) ? Long.parseLong(splits[i+1]) : Long.MAX_VALUE);
      if(keys != null)
        break;
    }
    if(keys == null)
...
public int run(String[] args, Configuration conf, ProviderProperties providerProperties)
{
  try
  {
    ...
    List<Long> keys2 = getRandom(keyFile,Integer.parseInt(parser.getOptionValue("tiles")), minTile, maxTile, new Random());
    for(Long key : keys2) {
      System.out.println(key);
